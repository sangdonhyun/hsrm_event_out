...
class itsm_event():
    ...
    def get_conn_info(self, target='mariadb'):
        opt = self.cfg.options('mariadb')
        conn_info = dict()
        print(opt)
        for k in opt:
            conn_info[k] = self.cfg.get('mariadb',k)
        return conn_info

    def send_mariadb(self,msg):
        conn_info = self.get_conn_info('mariadb')
        # Connect to MariaDB Platform
        if len(conn_info['passwd']) > 15:
            conn_info['passwd'] = self.o_aes.decrypt(conn_info['passwd'])
        try:
        ...
        evt_info = dict()
        evt_info['MSG_TYPE'] = '0'
        evt_info['CMID'] = datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')
        evt_info['DEST_PHONE'] = '01011112222'
        evt_info['SEND_PHONE'] = '01033334444'
        evt_info['MSG_BODY'] = msg
        cur = conn.cursor()
        ...
        """ (0, now(6), NOW(), NOW(),'01012341234', '0212341234', '본 메시지는 SMS 테스트 메시지 입니다.')"""
        sql = """INSERT INTO event.BIZ_MSG (MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE, MSG_BODY)
        VALUES (%s, DATE_FORMAT(NOW(6),'%Y%m%d%H%i%s%f'), NOW(), NOW(),%s, %s, %s)
        """
        print(sql,(evt_info['MSG_TYPE'],
                          evt_info['DEST_PHONE'], evt_info['SEND_PHONE'],
                          evt_info['MSG_BODY']))
        cur.execute(sql, (evt_info['MSG_TYPE'],
                          evt_info['DEST_PHONE'], evt_info['SEND_PHONE'],
                          evt_info['MSG_BODY']))
        ...
        for evt_info in evt_list:
            ...
            if msg in log_str:
                ...
                if send_method == 'syslog':
                    ...
                    self.send_smtp( msg)
                elif send_method == 'mariadb':
                    self.send_mariadb(msg)
                else:
                    self.send_file(msg)
